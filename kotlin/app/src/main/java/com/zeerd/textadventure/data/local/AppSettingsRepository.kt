...
class AppSettingsRepository @Inject constructor(
    ...
    companion object {
        private const val TAG = "TextAdventure-AppSettingsRepository"
    }
    // 偏好设置文件
    ...
    fun getSettings(): AppSettings {
        ...
        Log.v(TAG, "<<< getSettings() OUT - lang=${result.language}, backend=${result.backend}")
        return result
    }
    ...
    fun saveSettings(settings: AppSettings) {
        Log.v(TAG, ">>> saveSettings() IN - lang=${settings.language}, backend=${settings.backend}")
        val json = gson.toJson(settings)
        sharedPreferences.edit().putString(settingsKey, json).apply()
        Log.v(TAG, "<<< saveSettings() OUT")
    }
     ...
     */
    fun resetSettings() {
        Log.v(TAG, ">>> resetSettings() IN")
        sharedPreferences.edit().remove(settingsKey).apply()
        Log.v(TAG, "<<< resetSettings() OUT")
    }
}
