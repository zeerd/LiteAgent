package com.zeerd.textadventure.data.local
...
class StorySettingsDataSource @Inject constructor() {
    companion object {
        private const val TAG = "TextAdventure-StorySettingsDataSource"
    }
    // 所有的故事背景设置列表
    ...
    init {
        Log.v(TAG, ">>> StorySettingsDataSource init IN")
        // 初始化时加载预设的数据
        initializeStorySettings()
        Log.v(TAG, "<<< StorySettingsDataSource init OUT")
    }

    private fun initializeStorySettings() {
        Log.v(TAG, ">>> initializeStorySettings() IN")
        val settings = listOf(
            StorySetting(
                id = "fantasy",
                title = "奇幻冒险",
        ...
        Log.v(TAG, "<<< initializeStorySettings() OUT - loaded ${settings.size} settings")
    }
    ...
    fun selectSetting(setting: StorySetting) {
        Log.v(TAG, ">>> selectSetting() IN - setting=${setting.title}")
        _selectedSetting.value = setting
        Log.v(TAG, "<<< selectSetting() OUT")
    }
     ...
     */
    fun clearSelection() {
        Log.v(TAG, ">>> clearSelection() IN")
        _selectedSetting.value = null
        Log.v(TAG, "<<< clearSelection() OUT")
    }
    ...
    suspend fun getSettingById(settingId: String): StorySetting? {
        Log.v(TAG, ">>> getSettingById() IN - settingId=$settingId")
        val result = _storySettings.value.find { it.id == settingId }
        Log.v(TAG, "<<< getSettingById() OUT - ${if (result != null) "found" else "null"}")
        return result
    }
}
